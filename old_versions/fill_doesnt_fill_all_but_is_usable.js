    ...
    if (isNaN(totalPlayers) || totalPlayers < 8) {
        ...
        function fillEmptyFields() {
            ...
            let remainingNumbers = availableNumbers.filter(number => {
                ...
                let selectedCount = counts[number] || 0;
                return selectedCount < allowedCount && !numbersToAssign.includes(number);
            });

            // Calculate how many more numbers we need
            let emptyFields = selects.filter(select => !select.value).length;
            let numbersNeeded = emptyFields - numbersToAssign.length;

            // Add remaining numbers to numbersToAssign
            let totalAvailableNumbers = remainingNumbers.slice(0, numbersNeeded);
            numbersToAssign = numbersToAssign.concat(totalAvailableNumbers);

            // If we still need more numbers, add zeros and ones
            let extraNumbersNeeded = emptyFields - numbersToAssign.length;
            if (extraNumbersNeeded > 0) {
                let zerosAndOnes = [];
                for (let i = 0; i < extraZeros && zerosAndOnes.length < extraNumbersNeeded; i++) {
                    zerosAndOnes.push(0);
                }
                for (let i = 0; i < extraOnes && zerosAndOnes.length < extraNumbersNeeded; i++) {
                    zerosAndOnes.push(1);
                }
                numbersToAssign = numbersToAssign.concat(zerosAndOnes);
            }

            // Shuffle numbersToAssign
