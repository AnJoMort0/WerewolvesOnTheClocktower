...
const GMRoom = () => {
  ...
  // Auto-kill werewolves marked Incendiado (instant red X, source = piromaníaco)
  useEffect(() => {
    for (const [pid, effs] of Object.entries(playerEffects)) {
      if (effs.has("incendiado") && WEREWOLF_ROLES.includes(roleAssignments[pid])
          && !permanentlyDead.has(pid)
          && playerStatuses[pid] !== "dead-this-night"
          && playerStatuses[pid] !== "dead") {
  ...
  const endNight = async () => {
    ...
    // Werewolf incendiado victims die (red X)
    for (const [pid, effects] of Object.entries(newEffects)) {
      if (effects.has("incendiado") && WEREWOLF_ROLES.includes(roleAssignments[pid]) && !newPermanentlyDead.has(pid)) {
  ...
  const handleFaroleiroReveal = useCallback(() => {
    const limitedUseRoles: RoleId[] = ["e03", "v10", "v18", "m01", "s01", "m02", "m03", "v04", "v13", "v14"];
  ...
  const conditionKeys = useMemo(() => {
    ...
    // Lobisomem Vidente has charges (single-use)
    keys["lobisomemVidenteHasCharges"] = !lobisomemVidenteUsed;
                    ...
                    .map((player) => {
                      ...
                      const rowContent = (
                        ...
                        >
                          ...
                          {/* Lobisomem Vidente used */}
                          {roleId === "m02" && !isPermanentDead && (
                            <div className="flex gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                              <Checkbox
                                checked={lobisomemVidenteUsed}
                                onCheckedChange={() => setLobisomemVidenteUsed(!lobisomemVidenteUsed)}
                                className="h-4 w-4 border-primary data-[state=checked]:bg-primary"
                              />
                            </div>
                          )}
