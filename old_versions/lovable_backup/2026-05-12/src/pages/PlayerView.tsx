...
const PlayerView = () => {
  ...
  const [timerSync, setTimerSync] = useState<{ phase: "day" | "tribunal"; timeLeft: number; isRunning: boolean; timerDone: boolean } | null>(null);
                ...
                >
                  ...
                  {timerSync && (
                    <div className="bg-card/50 border border-border/30 rounded-lg p-3 text-center">
                      <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{timerSync.phase === "day" ? "Dia" : "Tribunal"}</p>
                      <div className={`font-display text-3xl tracking-wider ${timerSync.timeLeft <= 30 ? "text-destructive animate-pulse" : "text-foreground"}`}>
                        {String(Math.floor(timerSync.timeLeft / 60)).padStart(2, "0")}:{String(timerSync.timeLeft % 60).padStart(2, "0")}
                      </div>
                      {timerSync.timerDone && (
                        <p className="text-xs text-yellow-400 mt-1">⏰ Tempo esgotado!</p>
                      )}
                    </div>
                  )}
                </motion.div>
