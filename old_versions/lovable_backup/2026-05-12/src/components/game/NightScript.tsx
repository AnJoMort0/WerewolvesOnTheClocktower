...
export const NightScript = ({
  ...
  }, [roleAssignments, players, _permanentlyDeadPlayerIds, poisonedPlayerId, illusionPlayerId, countsAsEvilBeing]);
  ...
  const crowDynamicText = useMemo(() => {
    ...
    if (isCrowPoisoned) {
      // Always wrong: pick offset of -1 or +1 (never 0)
  ...
  }, [roleAssignments, alivePlayers, poisonedPlayerId, illusionPlayerId, _permanentlyDeadPlayerIds, countsAsEvilBeing]);
  ...
  const scriptLines = useMemo(() => {
    ...
    if (nightNumber === 1) {
      const filtered = firstNightScript.filter((l) => isLineRelevant(l, activeRoles, permanentlyDeadRoles, roleAssignments, _permanentlyDeadPlayerIds, profeciaGhostPlayerIds));
      if (filtered.length > 0) lines.push({ section: "Primeira Noite", items: filtered });
    } else if (nightNumber === 2) {
      const filtered2 = secondNightScript.filter((l) => isLineRelevant(l, activeRoles, permanentlyDeadRoles, roleAssignments, _permanentlyDeadPlayerIds, profeciaGhostPlayerIds));
      if (filtered2.length > 0) lines.push({ section: "Início da Segunda Noite", items: filtered2 });
      const filteredNormal = normalNightScript.filter(filterLine);
    ...
    return lines;
  }, [nightNumber, activeRoles, permanentlyDeadRoles, filterLine, roleAssignments, _permanentlyDeadPlayerIds, profeciaGhostPlayerIds]);
