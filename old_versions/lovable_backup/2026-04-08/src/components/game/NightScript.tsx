...
function isLineRelevant(
  ...
  // Special handling: check if at least one player with any required role is alive
  return line.requires.some((r) => {
    // Check if any player with this role is alive (not permanently dead)
    const playersWithRole = Object.entries(roleAssignments).filter(([, role]) => role === r);
    if (playersWithRole.length === 0) return false;
    return playersWithRole.some(([pid]) => !permanentlyDeadPlayerIds.has(pid));
  });
}
...
export const NightScript = ({
  ...
  const permanentlyDeadRoles = useMemo(() => {
    ...
    Object.entries(rolePlayers).forEach(([role, pids]) => {
      if (pids.every((pid) => _permanentlyDeadPlayerIds.has(pid))) {
  ...
  const scriptLines = useMemo(() => {
    ...
    if (nightNumber === 1) {
      const filtered = firstNightScript.filter((l) => isLineRelevant(l, activeRoles, permanentlyDeadRoles, roleAssignments, _permanentlyDeadPlayerIds));
      if (filtered.length > 0) lines.push({ section: "Primeira Noite", items: filtered });
    } else if (nightNumber === 2) {
      const filtered2 = secondNightScript.filter((l) => isLineRelevant(l, activeRoles, permanentlyDeadRoles, roleAssignments, _permanentlyDeadPlayerIds));
      ...
      const filteredNormal = normalNightScript.filter((l) => {
        if (!isLineRelevant(l, activeRoles, permanentlyDeadRoles, roleAssignments, _permanentlyDeadPlayerIds)) return false;
    ...
    return lines;
  }, [nightNumber, activeRoles, permanentlyDeadRoles, chamanCharges, shouldShowVidenteLine, foxDisabled, roleAssignments, _permanentlyDeadPlayerIds]);
