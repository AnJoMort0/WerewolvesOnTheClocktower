...
export function resolveKillerCard(
  ...
  if (source === "soldado") {
    ...
    return { image: role.image, label: getRoleLabel("v09", lang) };
  }
  ...
  if (illusionPlayerId) {
    ...
    if (illusionRole === source) {
      ...
      return { image: role.image, label: getRoleLabel("a06", lang) };
    }
  }
  // Tetanus victims tagged "v07"
  // v07-poisoned → poisoned cavaleiro variant
  if (source === "v07-poisoned") {
    return { image: ROLES["v07"].image, label: getRoleLabel("v07", lang) };
  }
  // Default: treat source as a role id
  const role = ROLES[source as RoleId];
  if (role) return { image: role.image, label: getRoleLabel(source as RoleId, lang) };
