    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-display text-sm tracking-widest uppercase text-muted-foreground">
          {gamePhase === "day" ? `Dia ${nightNumber}` : `Tribunal — Dia ${nightNumber}`}
        </h2>
      </div>
      ...
      {gamePhase === "day" && dayDeadNames.length > 0 && (
        <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-3 text-center">
          <p className="text-sm font-body text-destructive">
            ...
            {" "}morreu{dayDeadNames.length > 1 ? "ram" : ""} esta noite.
          </p>
        </div>
      )}
        ...
        {timerDone && (
          ...
          >
            <p className="font-display text-yellow-400 text-sm">
              {gamePhase === "day" ? "⏰ O tempo do Dia acabou!" : "⏰ O tempo do Tribunal acabou!"}
            </p>
          </motion.div>
        )}
      </div>
        ...
        <div className="space-y-2">
          <div className="bg-card/50 border border-border/30 rounded-lg p-3 text-center">
            <p className="text-sm font-body">
              Votos necessários para execução: <span className="font-display text-primary text-lg">{votesNeeded}</span>
            </p>
            <p className="text-xs text-muted-foreground">({alivePlayers} jogadores vivos)</p>
          </div>
