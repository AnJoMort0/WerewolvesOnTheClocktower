...
export const VidenteRevealModal = ({
  ...
  // When Vidente is poisoned, generate fake role assignments for dead players
  const fakeRoleMap = useMemo(() => {
    if (!isVidentePoisoned || deadPlayerIds.length === 0) return null;

    // Get all roles currently in play, excluding Vidente (e04) and the actual roles of dead players
    const inPlayRoles = Object.values(roleAssignments).filter((r) => r !== "e04");
    // Remove duplicates
    const uniqueInPlay = [...new Set(inPlayRoles)];
    // Remove the actual roles of the dead players
    ...
    return map;
  }, [isVidentePoisoned, deadPlayerIds, roleAssignments]);
            ...
            <div className="grid grid-cols-2 gap-4">
              {deadPlayerIds.map((pid) => {
                ...
                if (isVidentePoisoned && fakeRoleMap?.[pid]) {
                  // Poisoned: show fake role
                  displayRole = fakeRoleMap[pid];
                } else if (pid === illusionPlayerId) {
                  // Illusion: show Ilusionista
