...
const GMRoom = () => {
  ...
  const handlePlayerStatusChange = (playerId: string, newStatus: PlayerStatus, _source?: string) => {
    if (newStatus === "poisoned") {
      if (poisonedPlayerId === playerId) {
        ...
        // Track bruxa target for rabbit tamer
      ...
      // Cavaleiro resurrection: if cavaleiro is resurrected, also resurrect linked death
  ...
  const endNight = async () => {
    ...
    // Save current red X players for Vidente
    ...
    // Poison and illusion persist across nights — do NOT clear them
  ...
  const getListDragProps = (playerId: string) => {
    ...
    if (role === KILL_DRAG_ROLE) {
      // Check if any werewolf (non-white) is poisoned
  ...
  // Vidente reveal handler - also sends to player device via Supabase
  ...
  }, [roomId, roleAssignments, lastNightDeadPlayerIds, illusionPlayerId, isVidentePoisoned]);
